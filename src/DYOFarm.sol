...
contract DYOFarm is ReentrancyGuard, Ownable {
    ...
    event WithdrawRemainingRewards(uint256 amount);
    ...
    /**
     * @dev Withdraw excess rewards from native from this DYOFarm.sol
     *
     * Must only be called by the owner
     * Must only be called after the end time of the Nitro Pool
     */
    function withdrawReimaningRewards() external onlyOwner nonReentrant {
        require(block.timestamp > settings.endTime);
        uint256 remainingBalance = IERC20(rewardsToken1.token).balanceOf(address(this));
        _safeRewardsTransfer(rewardsToken1.token, msg.sender, remainingBalance);
        emit WithdrawRemainingRewards(remainingBalance);
    }
